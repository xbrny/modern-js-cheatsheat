...
Dalam JavaScript, ada 3 jenis kata kunci yang boleh digunakan untuk mendeklarasikan pemboleh ubah. Ianya adalah ```var```, ```let```, dan ```const```.

#### Short explanation

<!-- Variables declared with ```const``` keyword can't be reassigned, while ```let``` and ```var``` can. -->
Pemboleh ubah yang dideklarasikan menggunakan ```const``` tidak boleh diberikan nilai yang baru, manakala ```let```, ```var``` boleh.
...
Saya mencadangkan agar sentiasa mendeklarasikan pemboleh ubah menggunakan ```const```, dan menggunakan ```let``` jika nilainya perlu dimutasikan atau diubah jika perlu

<table>
  <tr>
    <th></th>
    <th>Skop</th>
    <th>Kebolehan ubah nilai</th>
    <th>Kebolehan dimutasi</th>
  ...
  <tr>
    ...
    <td>Block</td>
    <td>Tidak</td>
    <td><a href="#const_mutable_sample">Boleh</a></td>
    <td>Boleh</td>
  </tr>
  <tr>
    <th>let</th>
